...
async function dbExerciseCatalog(){
  if(!pool) return [];
  try{
    // Read the complete row so the catalogue remains compatible with
    // installations where optional exercise columns were introduced
    // by different migration revisions.
    const r=await pool.query(
      `SELECT * FROM exercises WHERE status='PUBLISHED' ORDER BY id`
    );
    return r.rows;
  }catch(e){
    console.error('dbExerciseCatalog failed:', e.message);
    return [];
  }
}
...
app.post('/api/ai/daily-training-library',auth,async(req,res)=>{
 try{
   ...
   const minutes=Math.max(15,Math.min(90,Number(req.body?.availableMinutes||45)));
   const fallbackExercises=catalog.slice(0,minutes>=60?6:minutes>=40?6:4).map((c,i)=>({
     exerciseId:String(c.id),
     name:c.title||c.name||`Exercício ${i+1}`,
     sets:Number(c.sets||3),
     reps:Number(c.reps||8),
     durationSeconds:Number(c.duration_seconds||0),
     restSeconds:Number(c.rest_seconds||45),
     coachingCue:c.objective||c.description||'Executar com técnica controlada e qualidade.',
     videoRequired:false,
     videoUrl:c.video_url||null,
     videoStatus:c.video_status||'MISSING'
   }));

   // The training library must remain usable even when no OpenAI key is configured.
   // AI is an enhancement; it must not be a single point of failure for training.
   if(!process.env.OPENAI_API_KEY){
     const training={
       title:'Treino personalizado',
       focus:player.goal||'Desenvolvimento técnico',
       durationMinutes:minutes,
       intensity:'MODERATE',
       warmupMinutes:8,
       cooldownMinutes:5,
       exercises:fallbackExercises,
       safetyNotes:['Adequar a intensidade à idade e ao estado do jogador.','Parar perante dor ou desconforto anormal.'],
       rationale:'Treino criado automaticamente a partir da biblioteca publicada. A personalização por IA será aplicada quando o provedor estiver disponível.'
     };
     return res.json({provider:'library-fallback',model:null,training,usage:null});
   }

   const input={player,availableMinutes:minutes,recentExerciseResults:results.rows,recentSessionFeedback:feedback.rows,
   ...
   try{
     const result=await openAIResponses(
       [{role:'user',content:JSON.stringify(input)}],
       {instructions:'Generate today football training using ONLY exercise IDs from exerciseCatalog. Never invent an exerciseId or video URL. Return JSON with title, focus, durationMinutes, intensity, exercises (exerciseId,name,sets,reps,durationSeconds,restSeconds,coachingCue,videoRequired), safetyNotes, rationale. Keep age-appropriate volume. Do not diagnose injuries.',maxOutputTokens:2600}
     );
     let training; try{training=JSON.parse(result.text)}catch(_){throw new Error('AI_INVALID_JSON')}
     ...
     if(!training.exercises.length) throw new Error('AI_RETURNED_NO_LIBRARY_EXERCISES');
     await saveAIDecision(req.user.sub,'DAILY_TRAINING_LIBRARY_GUARD',input,training,result.model);
     return res.json({provider:'openai',model:result.model,training,usage:result.usage});
   }catch(aiError){
     console.error('AI unavailable; using library fallback:',aiError.message);
     const training={
       title:'Treino personalizado',focus:player.goal||'Desenvolvimento técnico',durationMinutes:minutes,intensity:'MODERATE',
       warmupMinutes:8,cooldownMinutes:5,exercises:fallbackExercises,
       safetyNotes:['Adequar a intensidade à idade e ao estado do jogador.','Parar perante dor ou desconforto anormal.'],
       rationale:'Treino criado a partir da biblioteca publicada porque a IA não está disponível neste momento.'
     };
     return res.json({provider:'library-fallback',model:null,training,usage:null});
   }
 }catch(e){console.error(e);res.status(500).json({error:'TRAINING_GENERATION_FAILED',message:e.message});}
});

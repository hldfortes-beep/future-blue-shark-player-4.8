...
app.post('/api/ai/daily-training-smart',auth,async(req,res)=>{
 try{
   ...
   // Offline fallback: the training must work even when OpenAI is not configured.
   // Use the already-ranked published exercise library instead of blocking the player.
   if(!process.env.OPENAI_API_KEY){
     const fallbackExercises=ranked.slice(0,6).map((c,i)=>({
       exerciseId:c.id,
       name:c.title||c.name||`Exercício ${i+1}`,
       sets:c.sets||3,
       reps:c.reps||8,
       durationSeconds:c.duration_seconds||0,
       restSeconds:c.rest_seconds||45,
       coachingCue:c.coaching_cue||'Executa com controlo, qualidade técnica e intensidade adequada à idade.',
       videoRequired:Boolean(c.video_url),
       videoUrl:c.video_url||null,
       videoStatus:c.video_status||'PUBLISHED',
       smartScore:c.smartScore
     }));
     const training={
       title:'Treino personalizado',
       focus:player.goal||player.objective||'Desenvolvimento técnico',
       durationMinutes:Math.min(45,Math.max(20,Math.round((req.body?.availableMinutes||45)))),
       intensity:'MODERATE',
       exercises:fallbackExercises,
       safetyNotes:['Respeitar a idade e a condição do jogador.','Parar se surgir dor ou mal-estar.'],
       rationale:'Treino gerado a partir da biblioteca de exercícios publicada e do perfil do jogador, sem depender da IA.'
     };
     await saveAIDecision(req.user.sub,'DAILY_TRAINING_SMART_FALLBACK',input,training,'deterministic');
     return res.json({provider:'deterministic',model:'library-fallback',training});
   }

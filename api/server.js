...
async function dbExerciseCatalog(){
  if(!pool) return [];
  try{
    const r=await pool.query(`SELECT id,title,name,position,age_band,level,domain,difficulty,video_url,video_status
      FROM exercises WHERE status='PUBLISHED' ORDER BY id`);
    return r.rows;
  }catch(_){ return []; }
}
